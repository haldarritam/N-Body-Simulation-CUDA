...
char char_buffer[]= "Time per frame: 0\n";
sf::Clock real_clock;
sf::Text text;
...
void *computeHost_SMT (void *arg)
{
	...
	for (unsigned long iter=0; iter<nIter; iter++) {
		...
		for (i=start; i<end; i++) {
			...
			body_graphics[i].move(*(*o_r + (ND*i)) - *(*i_r + (ND*i)) , *(*o_r + (ND*i+1)) - *(*i_r + (ND*i+1)));
		}
		...
		if (count == NUM_CPU_THREADS*(2*iter+2)) {
			...
			for (unsigned int idx=0; idx<nElem; idx++) {
				...
				fprintf(destFile, "%f,%f\n", *(*o_r+ND*idx+0), *(*o_r+ND*idx+1));
			}
