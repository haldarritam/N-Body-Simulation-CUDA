...
#elif CONFIG == 1
#include "aux_declarations_gpu_rtsim_2_multithreaded.h"
#elif CONFIG == 2
...
int main (const int argc, const char** argv) {
    ...
    body_pos *h_body_prev_pos, *d_body_prev_pos;
    body_dpos *h_body_dpos, *d_body_dpos;
    ...
    cudaMallocHost((body_pos **) &h_body_prev_pos, pos_bytes);
    cudaMallocHost((body_dpos **) &h_body_dpos, pos_bytes);
    ...
    for (i = 0; i < nBodies; i++) {
      h_body_prev_pos[i].x = h_body_pos[i].x;
      h_body_prev_pos[i].y = h_body_pos[i].y;
    }
    // Device side memory allocation  
    cudaMalloc((body_pos **) &d_body_pos, pos_bytes);
    cudaMalloc((body_pos **) &d_body_prev_pos, pos_bytes);
    cudaMalloc((body_dpos **) &d_body_dpos, pos_bytes);
    ...
    cudaMemcpy(d_body_prev_pos, h_body_prev_pos, pos_bytes, cudaMemcpyHostToDevice);
    ...
    while((window.isOpen()  && !stop)  || 
        ...
        nbody_integration<<<grid, block>>>(d_body_pos, d_body_prev_pos, d_body_dpos, d_body_par,dt,nBodies);
        cudaDeviceSynchronize();

        // memcopy (device -> host)
        cudaMemcpy(h_body_dpos, d_body_dpos, pos_bytes, cudaMemcpyDeviceToHost);

        double timeStampB = getTimeStamp();
        gpuErrchk(cudaPeekAtLastError());
        
        for (i = 0; i < nBodies; i++){
            body_graphics[i].move(h_body_dpos[i].dx, h_body_dpos[i].dy);
        }
...
__global__ void nbody_integration(body_pos* b_pos, body_pos* b_prev_pos, body_dpos* b_dpos, body_parameters* b_par, float dt, int n) {
	...
	if (idx) {
        ...
        b_dpos[idx].dx = b_pos[idx].x - b_prev_pos[idx].x;
        b_dpos[idx].dy = b_pos[idx].y - b_prev_pos[idx].y;
        
        b_prev_pos[idx].x = b_pos[idx].x;
        b_prev_pos[idx].y = b_pos[idx].y;
    }
}
