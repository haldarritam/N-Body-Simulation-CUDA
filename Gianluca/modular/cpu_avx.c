...
void *computeHost_SMT (void *arg)
{
	...
	unsigned long i, j;
	__m256 pos_i_avx, accel_ip1_avx, pos_j_avx, dpos_ip1_avx;
	...
	for (unsigned long iter=0; iter<nIter; iter++) {
		...
		for (i=start; i<end; i++) {
			pos_i_avx = _mm256_loadu_ps (*o_r + (ND*i+0));
			pos_i.x = *(*o_r + (ND*i+0));
			pos_i.y = *(*o_r + (ND*i+1));
			pos_i.z = *(*o_r + (ND*i+2));

			accel_ip1_avx = _mm256_setzero_ps();
			...
			j = 0;
			while (j < nElem) {
				pos_j_avx = _mm256_loadu_ps (*o_r + (ND*j+0));
				dpos_ip1_avx = _mm256_sub_ps (pos_j_avx, pos_i_avx);
				rDistSquared = _mm256_mul_ps (dpos_ip1_avx, dpos_ip1_avx)
